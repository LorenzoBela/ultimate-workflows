...
- **CoE 4-Check Integrity Audit:**
  1. **Reference Verification:** 100% of citations resolved via live registry/scholarly APIs (`context7`, Web Extraction Tool, `web_fetch_exa`). Zero phantom references allowed.
...
| Dimension | Tavily Search (Web Search / Deep Research Engine) | Exa Semantic Search (`web_search_exa`) |
|---|---|---|
...
#### A. Tavily Search (Web Search) — Exact API Signature Lookup
```json
{
...
*   **Action:**
    1. **Dual-Engine Simultaneous Execution:** Always execute search queries using BOTH Web Search (Web Search / Deep Research Engine) AND `exa` (`web_search_exa`) at the same time to collect multi-perspective search results and eliminate single-engine bias.
    ...
    5. **Site mapping & Discovery:** Use Sitemap Discovery Tool to discover URL structures while running Exa `web_search_exa` to find related conceptual links and competitor tools.
    6. **Deep crawling:** Use Web Crawler for comprehensive site crawling and Exa `web_fetch_exa` for semantic parsing.
    7. **Multi-step research:** Combine Deep Research Engine and Exa search simultaneously for multi-angle autonomous deep dives.
    8. Apply `Web Search & Intelligence Strategy` and Exa semantic search techniques in tandem for query construction, domain filtering, and parallel search orchestration.

### Phase 3: Targeted Extraction & Documentation (Tavily + Exa + Context7)
*   **Action:**
    1. **Content extraction:** Use Web Extraction Tool and `exa/web_fetch_exa` side-by-side to fetch and parse clean content/markdown from target URLs containing guides, tutorials, or API references.
    2. **Library documentation:** Use the two-step Context7 flow:
       *   Step 1: package documentation index — Resolve the library name to its Context7 ID.
       *   Step 2: official library documentation — Fetch version-specific API documentation with code examples.
...
*   **Action:**
    ...
    5. For stakeholder presentations, use Data Presentation & Deck Layouts for HTML slide decks.

---

## Tavily API Parameters & Configuration Guide

When calling the Web Search API tools programmatically or via subagents, configure parameters according to the following optimization guide:
...
When using the Library Documentation Retrieval server to retrieve official package documentation, execute the following two-step flow sequentially:

### Step 1: Resolve Library ID
Call package documentation index with the library name to retrieve the correct registry ID.
```json
// Example Arguments
{
  "libraryName": "date-fns"
...
Call official library documentation with the resolved ID to retrieve API markdown documentation, type stubs, and code examples.
```json
// Example Arguments
{
  "libraryId": "date_fns_v3",
...
When formulating search queries for Web Search or `web_search_exa`, use the following advanced search operators to filter results:
...
When extracting text from URLs using Web Extraction Tool or programmatic GET requests, clean the document corpus using these rules before running synthesis:
...
- **Tavily Search (Web Search / Deep Research Engine):** Best for structured domain filters, keyword indexing, deep site crawling, and comprehensive multi-query results.
...
### 1. Tavily Search Best Practices (`Web Search & Intelligence Strategy`)
...
### 4. Parallel Web Search Orchestrator (`Web Search & Intelligence Strategy`)
...
*   **Memory:** Use Persistent Project Memory / Scratchpad to persist research findings across conversations — never re-research the same topic.
