...
*   **MCP Tools:** `notebooklm/notebook_list`, `notebooklm/notebook_create`, `notebooklm/notebook_get`
*   **Sub-skills:** `concise-planning`
*   **Action:**
...
*   **MCP Tools:** `notebooklm/notebook_add_url`, `notebooklm/notebook_add_text`, `notebooklm/notebook_add_drive`, `notebooklm/notebook_describe`
*   **Action:**
...
*   **MCP Tools:** `notebooklm/source_describe`, `notebooklm/source_get_content`, `notebooklm/research_start`, `notebooklm/research_import`
*   **Action:**
...
*   **MCP Tools:** `notebooklm/audio_overview_create`
*   **Action:**
...
*   **MCP Tools:** `memory` (create_entities, create_relations, add_observations)
*   **Sub-skills:** `ultimate-documentation-workflow`
*   **Action:**
...
2. **Step 2 (Trigger Handoff):** Activate `/ultimate-research-workflow` using `tavily_search` / `perplexity_ask` / `search_web`.
...
|---|---|---|
| **Bulk uploading HTML pages** | Pulls in navbar, footer, and sidebar links, causing garbage responses. | Use clean markdown extraction (e.g. `tavily_extract`) before uploading. |
...
| **Ignoring local memory graph** | If you don't save findings locally, you re-run expensive queries. | Log key details in the local `memory` MCP server after research. |
...
## Local Memory Graph Integration (`memory` MCP)
