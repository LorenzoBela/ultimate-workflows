...
*   **Sub-skills:** `systematic-debugging`, `lint-and-validate`
*   **Action:**
    ...
    3. Run `lint-and-validate` first — many "bugs" are simply syntax, type, or lint errors.

### Step 2: Reproduction & Minimization
*   **Sub-skills:** `systematic-debugging`, `superpowers-tdd`
*   **MCP Tools:** `playwright` (browser_navigate, browser_console_messages, browser_network_requests, browser_snapshot)
*   **Action:**
...
*   **MCP Tools:** `sequential-thinking/sequentialthinking`, `supabase-mcp-server/execute_sql`, `supabase-mcp-server/get_logs`, `prisma-mcp-server/Prisma-Studio`, `prisma-mcp-server/migrate-status`
*   **Sub-skills:** `systematic-debugging`, `postgres-best-practices`
*   **Action:**
    ...
    5. Apply `supabase-postgres-best-practices` for database-specific root cause analysis.

### Step 4: Hypothesis Testing (Scientific Method)
*   **MCP Tools:** `sequential-thinking/sequentialthinking`, `perplexity-ask/perplexity_ask`
*   **Action:**
...
*   **Sub-skills:** `superpowers-tdd`, `ponytail`, `kaizen`, `lint-and-validate`
*   **Action:**
    ...
    4. **Verify no collateral damage:**
       ...
       *   Run `lint-and-validate` — zero new lint or type errors.
...
*   **MCP Tools:** `memory` (add_observations, create_entities)
*   **Sub-skills:** `caveman-review`, `caveman-commit`
*   **Action:**
    ...
    3. Use `caveman-review` for terse, actionable review of the fix.
    4. Use `caveman-commit` for a conventional, descriptive commit message: `fix(scope): brief description of root cause`.
...
ERROR OCCURS
    ...
    YES
    │
    ├── Is it a compile/type error? ── YES ──→ Run lint-and-validate → Fix types
    │
    NO
    │
...
### 2. Database Diagnostics (`postgres-best-practices`)
...
### 3. Repro Script Design & Profiling (`superpowers-debug`)
...
### 4. Metro Dev Server Profiling on Mobile (`react-native-best-practices`)
...
*   **Research:** Use `tavily-search`, `tavily-extract`, `perplexity-ask`, and `context7/get-library-docs` for researching error messages, known issues, and framework-specific debugging guides.
*   **Browser Debugging:** Use `playwright` MCP for reproducing and diagnosing browser-based issues (DOM state, network requests, console errors, screenshots).
...
*   **Memory:** Use `memory` MCP to persist bug patterns and root causes — build a project-specific bug knowledge base.
