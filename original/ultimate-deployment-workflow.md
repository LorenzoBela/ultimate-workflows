...
*   **MCP Tools:** `firebase-mcp-server/firebase_get_environment`, `supabase-mcp-server/get_project`
*   **Action:**
    ...
    5. Run `lint-and-validate` to ensure the codebase is error-free before building artifacts.

### Phase 2: Build & Containerization (Docker / CI)
*   **Sub-skills:** `ultimate-git-workflow`, `caveman-commit`
*   **Action:**
    ...
    2. Define CI configurations (GitHub Actions, GitLab CI): run linters (`lint-and-validate`), typecheck scripts, and integration tests before packaging artifacts.
    3. Pin base image tags (e.g. `node:20.11-alpine` instead of `node:latest`) to guarantee build reproducibility.
    4. Use `caveman-commit` for conventional, terse commit messages on deployment branches.

### Phase 3: Deployment Execution (Vercel / Supabase / Firebase / Cloud)
*   **MCP Tools:** `supabase-mcp-server/deploy_edge_function`, `firebase-mcp-server/firebase_deploy`, `supabase-mcp-server/apply_migration`, `prisma-mcp-server/migrate-dev`
*   **Sub-skills:** `upstash-qstash-js`, `upstash-workflow-js`
*   **Action:**
...
*   **Sub-skills:** `systematic-debugging`, `upstash-redis-js`
*   **MCP Tools:** `supabase-mcp-server/get_logs`, `firebase-mcp-server/firebase_deploy_status`
*   **Action:**
...
*   **Research:** Use `tavily-search` and `context7/get-library-docs` for CI/CD platform documentation and Docker best practices.
*   **Memory:** Use `memory` MCP to persist deployment configurations, rollback procedures, and incident responses across conversations.
